...
/* Dispose Item Node:
    Takes in an item node, frees it, and nullifies its pointers. */
...
void delete_item(FILE *ofp, tree_name_node *t, char *categoryBuf, char *typeBuf)
{
    tree_name_node *tree = find_tree_name(t, categoryBuf);
    if(tree == NULL)
    {
    ...
    else if(item->left == NULL || item->right == NULL)
    {
        ...
        if(item->left == NULL)
        {
            ...
            dispose_item_node(item->right);
            item->right->iparent->right = NULL;
        ...
        if(item->right == NULL)
        {
            ...
            dispose_item_node(item->left);
            item->left->iparent->left = NULL;
    ...
    else
    {
        ...
        descendant->iparent->left = NULL;
        dispose_item_node(descendant);
...
void process_command(char *ifileBuf, FILE *ofp, char *commandName, tree_name_node *t)
{
    ...
    {
        sscanf(ifileBuf, "%s %s %s", temp, categoryBuf, typeBuf);
        delete_item(ofp, t, categoryBuf, typeBuf);
    }
...
int main()
{
    ...
    FILE *ifp = fopen("cop3502-as4-sample-input.txt", "r");
    FILE *ofp = fopen("cop3502-as4-output-grimes-katelyn", "w");

    char ifileBuf[256];
    char commandName[256];
    int numTree = 0, numItems = 0, numCommands = 0;
